...
function makeSlider(node,opt){
  ...
  let dragging=null, grabDx=0, armed=null, api;
  ...
  /* Tap versus drag, not marker versus track.
     A press anywhere on the row only ARMS a drag — nothing moves yet. The drag begins
     on the first movement past a few pixels, so a tap (or a finger that is really
     trying to scroll the panel) never nudges the value. Land on the marker and it
     keeps the offset you grabbed it by; land on the track and it comes to your finger
     on that first move, which is what scrubbing should feel like. */
  const SLOP=3;
  function thumbAt(e){
    const t=e.target;
  ...
  function begin(which,fromThumb,cx){
    dragging=which;
    node.classList.add('is-drag');
    if(fromThumb){
      const tr=(which==='b'?thB:thA).getBoundingClientRect();
      grabDx=cx-(tr.left+tr.width/2);
    }else grabDx=0;
  }
  function down(e){
    if(e.button!=null && e.button!==0) return;
    const onThumb=thumbAt(e);
    /* on the track of a two-handled slider, the nearer marker is the one you meant */
    let which=onThumb;
    if(!which && dual){
      const v=fromX(e.clientX);
      which = Math.abs(v-va)<=Math.abs(v-vb) ? 'a' : 'b';
    }else if(!which) which='a';
    armed={ which:which, onThumb:!!onThumb, x:e.clientX };
    node.setPointerCapture&&node.setPointerCapture(e.pointerId);
    /* no paint, no callback: until the finger moves this is still just a tap */
  }
  function move(e){
    if(!dragging){
      if(!armed) return;
      if(Math.abs(e.clientX-armed.x)<SLOP) return;   /* still a tap */
      begin(armed.which,armed.onThumb,armed.x);
    }
  ...
  function up(e){
    ...
    if(!wasDragging) return;                 /* a tap changed nothing to commit */
